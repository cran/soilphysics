iwc <- 
function(theta_R, theta_S, alpha, n, a, b, graph = TRUE,
	xlab = "Matric potential (hPa)", ylab = "Water content", ...)
{
    ...
    Kr <- function(x, theta_R, theta_S, alpha, n) {
       out <- (1 - (alpha*x)^(n-1) * (1 + (alpha*x)^n)^(1/n - 1) )^2 *
          (1 + (alpha*x)^n)^( (1 - n) / 2*n)
       return(out)
    }
    ...
    {
        m <- 1 - 1/n
        out <- (( (theta_S - theta_R) / (theta - theta_R) )^(1/m) - 1 )^(1/n) * 1/alpha
        return(out)
    }
    ...
    Ea <- function(x, theta_R, theta_S, alpha, n) {
        out <- wa(x) * der(x, theta_R, theta_S, alpha, n)
        return(out)
    }

    EKa <- function(x, theta_R, theta_S, alpha, n) {
        out <- wK(x, theta_R, theta_S, alpha, n) * wa(x) * 
        der(x, theta_R, theta_S, alpha, n)
        return(out)
    }
    ...
    Kdry <- Kr(x = seq(12000, 15000, by = 50), theta_R, theta_S, alpha, n)
    fit2 <- fitsoilwater4(theta = Kdry, psi = seq(12000, 15000, by = 50), 
        model = "Ross")
    ...
    EKdry <- function(x, d, theta_R, theta_S, alpha, n) 
    {
        out <- wKdry(x, d) *
        der(x, theta_R, theta_S, alpha, n)
        return(out)
    ...
    ave1 <- trapez(x = seq(h10, h15, length = 1000), 
        y = EKa(x = seq(h10, h15, length = 1000), theta_R, theta_S, alpha, n))

    # EK
    ave2 <- trapez(x = seq(h15, 330, length = 1000), 
        y = EK(x = seq(h15, 330, length = 1000), theta_R, theta_S, alpha, n))

    # Ch
    ave3 <- trapez(x = seq(330, h1.5, length = 1000), 
        y = der(x = seq(330, h1.5, length = 1000), theta_R, theta_S, alpha, n))
    ...
    ave1h <- trapez(x = seq(h10, h15, length = 1000), 
        y = hEKa(x = seq(h10, h15, length = 1000), theta_R, theta_S, alpha, n))
    ...
    ave2h <- trapez(x = seq(h15, 330, length = 1000), 
        y = hEK(x = seq(h15, 330, length = 1000), theta_R, theta_S, alpha, n))
    ...
    ave3h <- trapez(x = seq(330, h1.5, length = 1000), 
        y = hder(x = seq(330, h1.5, length = 1000), theta_R, theta_S, alpha, n))
